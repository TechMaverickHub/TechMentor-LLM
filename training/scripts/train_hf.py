...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TechMentor with HF + PEFT QLoRA.")
...
def load_model_and_tokenizer(training: TrainingConfig):
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "Use a Colab T4 or Linux machine with NVIDIA GPU."
        )
    ...
    model, tokenizer = load_model_and_tokenizer(training)
